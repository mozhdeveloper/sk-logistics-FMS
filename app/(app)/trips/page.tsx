  ...
  const filtered = useMemo(() => trips.filter((t) => {
    if (search) {
      ...
      if (
        ...
        !(t.consigneeName?.toLowerCase().includes(q)) &&
    ...
    return true;
  }), [trips, search, statusFilter, clients]);
      ...
      <Card>
        <CardContent className="p-4 flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search ID, DR#, client, item, consignee..." className="pl-10" />
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="text-left text-xs uppercase text-muted-foreground border-b border-brand-border bg-gray-50/50">
                ...
                <th className="py-3 px-4 font-medium">Consignee</th>
              ...
              <tbody>
                {filtered.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="border-b border-brand-border/60 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4">{t.consigneeName || <span className="text-muted-foreground">—</span>}</td>
                      <td className="py-3 px-4">
                        {partner ? (
                ...
                {filtered.length === 0 && <tr><td colSpan={11} className="py-12 text-center text-muted-foreground">No trips found.</td></tr>}
              </tbody>
            </table>
          </div>
        </CardContent>
